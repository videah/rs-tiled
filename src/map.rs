...
pub struct Map<I = GgezImage> {
    ...
    pub tile_width: u32,
    pub tile_height: u32,
    pub tilesets: Vec<Tileset<I>>,
    pub layers: Vec<Layer>,
    pub image_layers: Vec<ImageLayer<I>>,
...
impl Map<GgezImage> {
    pub(crate) fn new<R: Read, P: AsRef<Path>>(
        ...
        parse_tag!(parser, "map",
                   "tileset" => |attrs| {
                        tilesets.push(try!(Tileset::<GgezImage>::new(parser, attrs, ctx, map_path.as_ref())));
                        Ok(())
                   },
                   "layer" => |attrs| {
                        layers.push(try!(Layer::new(parser, attrs, w)));
                   ...
                   "imagelayer" => |attrs| {
                        image_layers.push(try!(ImageLayer::<GgezImage>::new(parser, attrs, ctx, map_path.as_ref())));
                        Ok(())
                   },
                   "properties" => |_| {
                        properties = try!(parse_properties(parser));
...
impl<I> Map<I> {
    ...
    pub fn get_tileset_by_gid(&self, gid: u32) -> Option<&Tileset<I>> {
